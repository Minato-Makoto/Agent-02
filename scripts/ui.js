...
function resolveRunner() {
  ...
  if (pnpm) {
    return { cmd: pnpm, baseArgs: [], kind: "pnpm" };
  }
  const corepack = which("corepack");
  if (corepack) {
    const shimPath = ensurePnpmShim();
    if (shimPath) {
      return { cmd: shimPath, baseArgs: [], kind: "pnpm-shim" };
    }
  }
  return null;
}
...
export function main(argv = process.argv.slice(2)) {
  ...
  if (action === "install") {
    run(runner.cmd, [...runner.baseArgs, "install", ...rest]);
    return;
  }
  ...
  if (!depsInstalled(action === "test" ? "test" : "build")) {
    ...
    runSync(runner.cmd, [...runner.baseArgs, ...installArgs], installEnv);
  }

  run(runner.cmd, [...runner.baseArgs, "run", script, ...rest]);
}
